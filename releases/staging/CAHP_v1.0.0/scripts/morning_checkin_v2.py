...
def verify_release_artifact():
    ...
    if not os.path.exists(full_path):
        log(f"Release artifact missing: {full_path}", datetime.utcnow().isoformat() + "Z", "ERROR", "FAIL")
        return False
    ...
    if calculated_hash == EXPECTED_HASH:
        log(f"Release integrity verified ({calculated_hash[:8]}...)", datetime.utcnow().isoformat() + "Z", "VERIFIED", "PASS")
        return True
    else:
        log(f"Release hash mismatch! Expected {EXPECTED_HASH}, got {calculated_hash}", datetime.utcnow().isoformat() + "Z", "ERROR", "FAIL")
        return False
...
def sign_pulse(status, timestamp, tag):
    """
    Creates a signed Pulse Certificate for MNAP-001.
    """
    if not os.path.exists(KEY_PATH):
        log("L0 Identity Key not found. Cannot sign pulse.", "FAIL")
        return False
    ...
    filename = f"pulse_certificate_{datetime.utcnow().strftime('%Y_%m_%d')}.json"
...
def main():
    ...
    print(f"{COLORS['YELLOW']}--- HELIX GOOSE NODE MORNING CHECK-IN (v2 - SIGNED) ---{COLORS['RESET']}")
    ...
    # Summary
    print("--- SUMMARY ---")
    if assets_ok and tests_ok and gov_ok and mimicry_ok:
